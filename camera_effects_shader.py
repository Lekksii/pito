...
uniform float c_G = 255;
uniform float c_B = 255;
...
vec4 fxaa(sampler2D tex, vec2 fragCoord, vec2 resolution) {
  ...
  float rcpDirMin = 1.1 / (min(abs(dir.x), abs(dir.y)) + dirReduce);
...
void main () {
  ...
  //float gray = color.r*.3 + color.g*.59 + color.b*.11;
  
  color = vec4(color.r/c_R, color.g/c_G, color.b/c_B, 1.0);
